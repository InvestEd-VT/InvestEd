...
export function TradeModal({
  ...
  return (
    <Sheet open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <SheetContent
        className="w-[340px] sm:max-w-[380px] bg-white border-gray-200 p-0 flex flex-col overflow-y-auto"
        aria-label={`${mode === 'buy' ? 'Buy' : 'Sell'} ${contract.contract_type.toUpperCase()} option order`}
      >
              ...
              <div id="trade-order-summary" className="rounded-lg bg-gray-50 p-4 space-y-2">
                ...
                <div className="flex justify-between text-sm font-semibold">
                  <span className="text-gray-400">
                    Estimated {mode === 'buy' ? 'cost' : 'credit'}
                  </span>
