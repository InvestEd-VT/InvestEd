...
export default function PortfolioTrendCard({ defaultRange = '30d' }: { defaultRange?: string }) {
  ...
  useEffect(() => {
    ...
    portfolioService
      ...
      .then((res) => {
        if (!mounted) return
        const chart = (res.history || []).map((h: any) => ({ date: new Date(h.date).toISOString(), value: Number(h.cashBalance || 0) }))
        setData(chart)
      })
      .catch(() => {
        if (!mounted) return
        ...
        <ChartContainer config={{ value: { label: 'Value' } }} className="h-36">
          {loading ? (
            ...
            <AreaChart data={data}>
              ...
              />
              <Area dataKey="value" type="monotone" fill="var(--primary)" stroke="var(--primary)" fillOpacity={0.15} />
            </AreaChart>
          )}
        </ChartContainer>
        <div className="mt-2 text-xs text-muted-foreground">Values shown are transaction-based cash snapshots.</div>
      </CardContent>
    </Card>
  )
}
