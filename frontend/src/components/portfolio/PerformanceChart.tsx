...
import { LineChart, Line, XAxis, Tooltip, CartesianGrid, ReferenceLine, ResponsiveContainer } from 'recharts';
...
export default function PerformanceChart({ period = 'all' }: { period?: string }) {
  ...
  React.useEffect(() => {
    ...
    portfolioService
      ...
      .then((res) => {
        ...
        const history = (res.history || []).map((h: any) => ({ date: new Date(h.date).toISOString(), value: Number(h.cashBalance) }));
        setData(history);
  ...
  const lineColor = startingBalance != null && latestValue != null && latestValue >= startingBalance ? 'var(--emerald-600)' : 'var(--rose-600)';
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        ...
        >
          {loading ? (
            ...
            <div className="flex h-40 items-center justify-center text-sm text-muted-foreground">No historical data available</div>
          ) : (
                  ...
                  content={({ active, payload, label }: any) => {
                    ...
                    return (
                      <div className="rounded bg-white p-2 shadow">
                        ...
                        <div className="text-sm font-medium">{formatCurrency(Number(val ?? 0), 'USD')}</div>
                      </div>
                    );
                  }}
                />
                {startingBalance != null && (
                  <ReferenceLine y={startingBalance} stroke="var(--muted)" strokeDasharray="4 4" label={{ position: 'right', value: `Start: ${formatCurrency(startingBalance, 'USD')}`, fill: 'var(--muted)', fontSize: 12 }} />
                )}
