...
export default function TopPositionsList() {
  ...
  const optionPositions = positions
    ...
    .map((p) => ({
      ...p,
      currentValue: Number(p.marketValue ?? (p.currentPrice ? p.currentPrice * (p.quantity ?? 0) * (p.positionType === 'OPTION' ? 100 : 1) : 0)),
    }))
  ...
  if (!top5.length) {
    return (
      <Card className="@container/card">
        ...
        <CardContent>
          <div className="py-6 text-center text-sm text-muted-foreground">No open options positions</div>
        </CardContent>
        <CardFooter>
          <div className="w-full flex justify-end">
            <a href="/portfolio">
              <Button size="sm" variant="ghost">View all positions</Button>
            </a>
          </div>
        </CardFooter>
      </Card>
    ...
    <Card className="@container/card">
      ...
      <CardContent className="space-y-2">
        ...
        {top5.map((p) => {
          ...
          const expiration = (p as any).expiration || (p as any).expirationDate || (p as any).expiresAt || null
          ...
          return (
            <div key={p.id} className="grid grid-cols-12 items-center gap-2 px-2 py-2 border-b last:border-b-0">
              <div className="col-span-3 font-medium truncate">{p.symbol}</div>
              <div className="col-span-2">{p.optionType ?? p.positionType ?? '-'}</div>
              <div className="col-span-2">{p.strikePrice ? Number(p.strikePrice).toFixed(2) : '-'}</div>
              <div className="col-span-2">{expText}</div>
              <div className="col-span-1">{Number(p.quantity ?? 0)}</div>
              <div className="col-span-2 text-right">
                <div className="font-mono font-medium">{formatCurrency(Number(p.currentValue ?? p.marketValue ?? 0), 'USD')}</div>
                <div className={`text-sm ${positive ? 'text-green-600' : 'text-red-600'}`}>
                  {positive ? '+' : '-'}{formatCurrency(Math.abs(pnl), 'USD')}
                </div>
              </div>
            </div>
          )
